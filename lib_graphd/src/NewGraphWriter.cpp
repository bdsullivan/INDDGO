...
namespace Graph {
     ...
     * \param[in] filename full path of the file to be written to
     */
    int NewGraphWriter::write_adjmatrix(Graph *g, const string filename){
     ...
     * \return 0 on success, nonzero on failure
     */
    int NewGraphWriter::write_metis(Graph *g, const string filename){
     ...
     * \param[in] filename full path of the file to be written to
     */
    int NewGraphWriter::write_dimacs(Graph *g, const string filename, bool write_vertex_weights){
    ...
    int NewGraphWriter::write_graphviz(Graph *g, const string filename){
        int i, j;
        FILE *out;
        if((out = fopen(filename.c_str(), "w")) == NULL){

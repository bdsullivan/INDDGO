...
int parse_options(int argc, char **argv, string& infile, string& intype, string& outfilename, string &outprefix, std::map<string, bool>& methods, bool& record_timings, int *spectrum_spread){
    int flags, opt;
    while((opt = getopt(argc, argv, "hi:t:o:m:p:s:r")) != -1){
        switch(opt){
        case 'h':
            print_usage(argv);
            exit(0);
...
void run_all_methods(Graph::Graph *g, ofstream &outfile, ofstream &timing_file, string outprefix, std::map<string, bool> req_methods,  int spectrum_spread){
...
int main(int argc, char **argv){
    ...
    parse_options(argc, argv, infile, intype, outfilename, outprefix, req_methods, record_timings, &spectrum_spread);
    ...
    run_all_methods(g, outfile, timing_file, outprefix, req_methods, spectrum_spread);
    outfile.close();
    timing_file.close();
    ...
    if(not is_connected){  // run everything against the other algorithms
        ...
        run_all_methods(largest_component, outfile, timing_file, outprefix, req_methods, spectrum_spread);
        outfile.close();
        timing_file.close();
    }
